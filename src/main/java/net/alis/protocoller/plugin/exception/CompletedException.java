...
public class CompletedException {
    ...
    protected CompletedException(RuntimeException exception, boolean showStacktrace) {
    ...
    public <F> F throwException() {
        if(saveToFile) {
            TaskSimplifier.get().preformAsync(() -> {
                ...
                for(StackTraceElement traceElement : this.exception.getStackTrace()) manager.writeNewLine(traceElement.toString());
                manager.stopAll();
            });
        }
