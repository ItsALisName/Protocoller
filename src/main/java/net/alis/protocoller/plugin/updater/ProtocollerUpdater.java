...
public class ProtocollerUpdater {
    ...
    public static void init() {
        ...
        if(ProtocollerConfig.isAutoUpdateCheck()){
            if (ProtocollerConfig.isAsyncUpdateChecking()) {
                TaskSimplifier.get().preformAsyncTimerTask(ProtocollerUpdater::checkUpdate, 100L, (ProtocollerConfig.getAutoUpdateCheckInterval() * 20L));
            } else {
                TaskSimplifier.get().preformSyncTimerTask(ProtocollerUpdater::checkUpdate, 100L, (ProtocollerConfig.getAutoUpdateCheckInterval() * 20L));
            }
        }
    }
    ...
    private static void checkUpdate() {
        try {
            InputStreamReader input = new InputStreamReader(((HttpURLConnection) new URL(SPIGOT_API_LINK).openConnection()).getInputStream());
        ...
        if(!latest.equalsIgnoreCase(Protocoller.get().getVersion())) {
            if(ProtocollerConfig.isAutoUpdateDownload()) {
                ...
                try {
                    if(Bukkit.isPrimaryThread() && ProtocollerConfig.isAsyncFilesWorking()) {
    ...
    private static void notifyOperators(String message) {
        if(ProtocollerConfig.isNotifyOperatorsOnUpdate()) {
            for(Player player : Bukkit.getOnlinePlayers()) {
                if(player.isOp()) player.sendMessage(Utils.setColors(message));
            }
        }
    }

    public static void notifyOperator(Player player) {
        if(!currentVersionIsLatest && player.isOp()) {
            player.sendMessage(Utils.setColors("&c[Protocoller] You are using an old version of the plugin! Current version \"&4" + Protocoller.get().getVersion() + "&c\", latest version \"&4" + latest + "&c\", please update the plugin."));
        }
    }
