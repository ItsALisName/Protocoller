package net.alis.protocoller.plugin.v0_0_5.netty.interceptors;
...
public class NettyPacketInterceptor extends ChannelDuplexHandler {
    ...
    public NettyPacketInterceptor() {
        IProtocolAccess.get().getServer().getPacketInterceptors().add(this);
    }

    @Override
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        if(networkPlayer != null) ((ProtocolPlayer)networkPlayer).addSentPacket();
        ...
        if(!ProtocolUtil.viaVersionInstalled){
            if (data.getType() == PacketType.Handshake.Client.SET_PROTOCOL) {
                ...
                if (!ProtocolUtil.TEMP_PROTOCOL_MAP.containsKey(protocol.getAddress())) {
                    ...
                    ITaskAccess.get().doAsyncLater(() -> ProtocolUtil.TEMP_PROTOCOL_MAP.remove(protocol.getAddress()), 300L);
                }
            }
        }
        if((data.getType().getState() == PacketType.State.PLAY_SERVERBOUND || data.getType().getState() == PacketType.State.SERVERBOUND) && player == null) {
            super.channelRead(ctx, msg);
            return;
        }
    ...
    public void write(ChannelHandlerContext ctx, @NotNull Object msg, ChannelPromise promise) throws Exception {
        if(networkPlayer != null) ((ProtocolPlayer)networkPlayer).addReceivedPacket();
        if(msg.getClass().getSimpleName().length() < 5) {
        ...
        if((data.getType().getState() == PacketType.State.PLAY_CLIENTBOUND || data.getType().getState() == PacketType.State.CLIENTBOUND) && this.player == null) {
            super.write(ctx, msg, promise);
            return;
        }
        AsyncPacketEventManager.callListeners(data, ctx.channel(), (InetSocketAddress) ctx.channel().remoteAddress(), player, networkPlayer);
